  ...
  case "thread-turning":
    ...
    if (properties.useSimpleThread ||
      ...
      if (isLastCyclePoint()) {
        ...
        writeBlock(
          gCycleModal.format(codes[properties.type]),
...
function writeRetract() {
  ...
  */
  ...
  } else if (_useG28 == false) {
    // xHome = machineConfiguration.hasHomePositionX() ? machineConfiguration.getHomePositionX() : toPreciseUnit(0, MM);
    _xHome = properties.g53HomePositionX;
    _yHome = machineConfiguration.hasHomePositionY() ? machineConfiguration.getHomePositionY() : toPreciseUnit(0, MM);
    // zHome = machineConfiguration.getRetractPlane() != 0 ? machineConfiguration.getRetractPlane() : toPreciseUnit(0, MM); // what is this for a lathe
    _zHome = properties.g53HomePositionZ;
  }
  for (var i = 0; i < arguments.length; ++i) {
    switch (arguments[i]) {
    case X:
      words.push((_useG28 == true ? "U" : "X") + xyzFormat.format(_xHome));
      xOutput.reset();
      retracted = true;
      break;
    case Y:
      if (yOutput.isEnabled()) {
        words.push((_useG28 == true ? "V" : "Y") + xyzFormat.format(_yHome));
        yOutput.reset();
      }
      break;
    case Z:
      words.push((_useG28 == true ? "W" : "Z") + xyzFormat.format(_zHome));
      zOutput.reset();
      retracted = true;
      break;
    default:
