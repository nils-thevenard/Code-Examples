...
propertyDefinitions = {
  ...
  toolAsName: {
    title: "Tool as name",
    description:
      "If enabled, the tool will be called with the tool description rather than the tool number.",
    type: "boolean",
  },
  cycle800Mode: {
    title: "CYCLE800 mode",
  ...
  cycle800SwivelDataRecord: {
    ...
    description:
      "Specifies the label to use for the Swivel Data Record for CYCLE800.",
    type: "string",
  },
  singleLineProbing: {
    title: "Single line probing",
    description:
      "If enabled, probing will be output in a single cycle call line.",
    type: "boolean",
  },
};
...
*/
  ...
  // }
  writeln(
    "%_N_" + translateText(String(programName).toUpperCase(), " ", "_") + "_MPF"
  );
...
function isInspectionOperation(section) {
  return (
    ...
    section.getParameter("operation-strategy") == "inspectSurface"
  );
}
...
function setWorkPlane(abc, turn) {
  ...
  if (useMultiAxisFeatures) {
    ...
    writeBlock("A" + abcFormat.format(A));
  } else {
    gMotionModal.reset();
    writeBlock(
...
function onSection() {
  if (properties.toolAsName && !tool.description) {
    if (hasParameter("operation-comment")) {
      error(
        subst(
          localize('Tool description is empty in operation "%1".'),
          getParameter("operation-comment").toUpperCase()
        )
      );
    } else {
  ...
  var insertToolCall =
    ...
    conditional(
      properties.toolAsName,
      tool.description != getPreviousSection().getTool().description
    );
  ...
  if (insertToolCall) {
    ...
    if (properties.preloadTool) {
      var nextTool = properties.toolAsName
        ? getNextToolDescription(tool.description)
        : getNextTool(tool.number);
      if (nextTool) {
        writeBlock(
          "T" +
            (properties.toolAsName
              ? "=" + '"' + nextTool.description.toUpperCase() + '"'
  ...
  if (currentSection.isMultiAxis()) {
    ...
    if (insertToolCall) {
      if (tool.lengthOffset != 0) {
        warningOnce(
          localize("Length offset is not supported."),
          WARNING_LENGTH_OFFSET
        );
      }
...
function getProbingArguments(cycle, singleLine) {
  ...
  return {
    ...
    isIncrementalDepth:
      cycleType.indexOf("island") != -1 ||
      cycleType.indexOf("wall") != -1 ||
      cycleType.indexOf("boss") != -1,
    ...
    _TUL: !isAngleProbing
      ? cycle.tolerancePosition
        ? (!singleLine ? "_TUL=" : "") +
          xyzFormat.format(cycle.tolerancePosition)
        : undefined
      : undefined,
    _TLL: !isAngleProbing
      ? cycle.tolerancePosition
        ? (!singleLine ? "_TLL=" : "") +
          xyzFormat.format(cycle.tolerancePosition * -1)
        : undefined
      : undefined,
    _TNUM:
      !isAngleProbing && cycle.updateToolWear
        ? (!singleLine ? (properties.toolAsName ? "_TNAME=" : "_TNUM=") : "") +
          (properties.toolAsName
            ? '"' + cycle.toolDescription.toUpperCase() + '"'
            : toolFormat.format(cycle.toolWearNumber))
        : undefined,
    _TDIF:
      !isAngleProbing && cycle.updateToolWear
        ? (!singleLine ? "_TDIF=" : "") +
          xyzFormat.format(cycle.toolWearUpdateThreshold)
        : undefined,
    _TMV: cycle.hasSizeTolerance
      ? !isAngleProbing && cycle.updateToolWear
        ? (!singleLine ? "_TMV=" : "") + xyzFormat.format(cycle.toleranceSize)
        : undefined
      : undefined,
    _KNUM:
      !isAngleProbing && cycle.updateToolWear
        ? (!singleLine ? "_KNUM=" : "") +
          xyzFormat.format(
            cycleType == "probing-z"
              ? 1000 + cycle.toolLengthOffset
              : 2000 + cycle.toolDiameterOffset
          )
        : isAngleProbing && !probeWCS
        ? (!singleLine ? "_KNUM=" : "") + 0
        : undefined, // 2001 for D1
  };
}
...
function onCyclePoint(x, y, z) {
  ...
  if (isProbeOperation()) {
    ...
    var _TSA =
      (!singleLine ? "_TSA=" : "") +
      (cycleType.indexOf("angle") != -1
        ? xyzFormat.format(0.1)
        : xyzFormat.format(1)); // tolerance (trust area) //angle tolerance (in the simulation he move to the second point with this angle)
    var _NMSP = (!singleLine ? "_NMSP=" : "") + xyzFormat.format(1); // number of measurements at same spot
    var _ID = probingArguments.isIncrementalDepth
      ? (!singleLine ? "_ID=" : "") + xyzFormat.format(cycle.depth * -1)
      : undefined; // incremental depth infeed in Z, direction over sign (only by circular boss, wall resp. rectangle and by hole/channel/circular boss/wall with guard zone)
    var _SETVAL = !probingArguments.isRectangularFeature
      ? !singleLine
        ? "_SETVAL="
        : ""
      : undefined;
    _SETVAL =
      cycle.width1 && !probingArguments.isRectangularFeature
        ? _SETVAL + xyzFormat.format(cycle.width1)
        : _SETVAL;
    var _SETV0 = probingArguments.isRectangularFeature
      ? (!singleLine ? "_SETV[0]=" : "") +
        (cycle.width1
          ...
          ? xyzFormat.format(0)
          : "")
      : undefined; // nominal value in X
    var _SETV1 = probingArguments.isRectangularFeature
      ? (!singleLine ? "_SETV[1]=" : "") +
        (cycle.width2 ? xyzFormat.format(cycle.width2) : "")
      : undefined; // nominal value in Y
    var _DMODE = 0;
    // _FA is always in mm
    var _FA =
      (!singleLine ? "_FA=" : "") + // measuring range (distance to surface), total measuring range=2*_FA in mm
      xyzFormat.format(
        (unit == MM ? 1 : 25.4) *
          (cycle.probeClearance ? cycle.probeClearance : cycle.probeOvertravel)
      );
    var _RA = probingArguments.isAngleProbing
      ? (!singleLine ? "_RA=" : "") + xyzFormat.format(0)
      : undefined; // correction of angle, 0 dont rotate the table;
    var _STA1 = probingArguments.isAngleProbing
      ? (!singleLine ? "_STA1=" : "") + xyzFormat.format(0)
      : undefined; // angle of the plane
    ...
    if (_KNUM == undefined) {
      _KNUM = !singleLine
        ? "_KNUM=" + xyzFormat.format(probeOutputWorkOffset)
        : xyzFormat.format(10000 + probeOutputWorkOffset); // automatically input in active workOffset. e.g. _KNUM=1 (G54)
    }

    if (!properties.toolAsName && tool.number >= 100) {
      error(
        localize(
          "Tool number is out of range for probing. Tool number must be below 100."
        )
      );
      return;
    }
    ...
    switch (cycleType) {
      ...
      case "probing-y":
        cycleParameters = {
          cycleNumber: 978,
          _MA: cycleType == "probing-x" ? 1 : 2,
          _MVAR: 0,
        };
        _SETVAL += xyzFormat.format(
          (cycleType == "probing-x" ? x : y) +
      ...
      case "probing-z":
        ...
        writeBlock(
          gMotionModal.format(1),
          zOutput.format(z - cycle.depth + cycle.probeClearance)
        );
        break;
      case "probing-x-channel":
        cycleParameters = { cycleNumber: 977, _MA: 1, _MVAR: 3 };
        writeBlock(
          gMotionModal.format(1) + " " + zOutput.format(z - cycle.depth)
        );
        break;
      ...
      */
      ...
      case "probing-y-wall":
        cycleParameters = {
          cycleNumber: 977,
          _MA: cycleType == "probing-x-wall" ? 1 : 2,
          _MVAR: 4,
        };
        break;
      ...
      case "probing-y-plane-angle":
        cycleParameters = {
          cycleNumber: 998,
          _MA: cycleType == "probing-x-plane-angle" ? 201 : 102,
          _MVAR: 5,
        };
        _ID =
          (!singleLine ? "_ID=" : "") + xyzFormat.format(cycle.probeSpacing); // distance between points
        _SETVAL += xyzFormat.format(
          (cycleType == "probing-x-plane-angle" ? x : y) +
            approach(cycle.approach1) *
              (cycle.probeClearance + tool.diameter / 2)
        );
        ...
        writeBlock(
          gMotionModal.format(1),
          cycleType == "probing-x-plane-angle"
            ? yOutput.format(y - cycle.probeSpacing / 2)
            : xOutput.format(x - cycle.probeSpacing / 2)
        );
        break;
      default:
        cycleNotSupported();
    ...
    var multiplier =
      probingArguments.probeWCS || probingArguments.isAngleProbing ? 100 : 0; // 1xx for datum shift correction
    multiplier =
      cycleType.indexOf("island") != -1 ? 1000 + multiplier : multiplier; // 1xxx for guardian zone
    var _MVAR =
      cycleParameters._MVAR != undefined
        ? (!singleLine ? "_MVAR=" : "") +
          xyzFormat.format(multiplier + cycleParameters._MVAR)
        : undefined; // CYCLE TYPE
    var _MA =
      cycleParameters._MA != undefined
        ? (!singleLine ? "_MA=" : "") + xyzFormat.format(cycleParameters._MA)
        : undefined;
    ...
    if (!singleLine) {
      ...
      writeBlock(
        "CYCLE" +
          xyzFormat.format(cycleParameters.cycleNumber) +
          "(" +
          procParam +
          cycleSeparator +
          ")"
    ...
    if (probingArguments.isOutOfPositionAction) {
      if (cycleParameters.cycleNumber != 977) {
        ...
        var positionLowerTolerance = xyzFormat.format(
          cycle.tolerancePosition * -1
        );
        writeBlock(
          "IF((_OVR[5]>" +
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      if (tool.diameterOffset != 0) {
        warningOnce(
          localize("Diameter offset is not supported."),
          WARNING_DIAMETER_OFFSET
        );
      }
    ...
    switch (getCircularPlane()) {
      case PLANE_XY:
        if (radiusCompensation != RADIUS_COMPENSATION_OFF) {
          if (
            gPlaneModal.getCurrent() !== null &&
            gPlaneModal.getCurrent() != 17
          ) {
            error(
              localize(
                "Plane cannot be changed when radius compensation is active."
              )
            );
            return;
          }
